...
// TODO: remove comments if not using preconditioner
...
f4) y = Minv * p_i - apply diagonal preconditioner to p_i (using element wise mult kernel)

f5) v_i = A*y - will use standard csrmv kernel (alpha = 1, beta = 0)

f6) alpha = rho_i / <rhat_0, v_i> - will use standard dot product kernel w/ modified final kernel finalizining <>
diving rho_i by it to calculate alpha

f7) s = r_im1 - alpha*v_i
h = x_im1 + alpha*y
Will both be done in single modified axpby where ops are r_im1 -= alpha*v_i and x_im1 += alpha*y
with r buffer now storing s_i values and x buffer now storing h values.
Kernel name will again need to be unique.

f8) norm(s) -> copy to host and check convergence - if converged, h is result, which is already stored in x buffer

f9) z = Minv * s - apply diagonal preconditioner to s (element wise mult kernel)
	z will replace y since that info is no longer needed

f10) t = A*z - uses standard csrmv kernel (alpha = 1, beta = 0)

f11) omega_i = <t,s>/<t,t> - Base kernels already made (dot and sqr), modified final kernel will calculate quotient
of resulting reduces and store in omega_i
...
f13) calc norm(r_i) -> copy to host and check convergence. If converged, x is solution. Uses standard norm reduce.
...
Kernels needed -
1) csrmv - only one needed for steps 2, f5, f8
2) modified axpby - one for step f3 and 1 for steps f6 and f10
1) norm base reduce for 1, f7 and f11
1) dot base reduce for f1, f5 and f9
1) sqr base reduce of 4, and f9
