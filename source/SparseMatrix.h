...
{
public:
	enum { C = 0, E = 1, W = 2, N = 3, S = 4 }; // Enum of indicies in *IndArray (i.e. east neighbor of element (i,j) = neigh_index(i+j*XsizeFull,E)) 

	bool iniFlag = false; // used to ensure that BiCGStabSolver initialization is only
						  // performed once on this class (setting names, allocating buffers,
...
// Pointer to vls.nType
	...
	bool testSolidNode(const int i, const int j);
	bool testSolidBoundaryNode(const int i, const int j);
	bool testFluidNode(const int i, const int j);
