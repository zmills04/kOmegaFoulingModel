...
public:
	...
	// pass copyLoc = false to skip copy of loc array
	void Par::copyToArrayOnDeviceBlocking(Par& Ptemp, arrName arrname_, int writeSize = -1,
		cl_command_queue* que_ = nullptr, int num_wait = 0, cl_event* wait = NULL);
...
typedef struct legacyBLinks
{
	...
	cl_ushort2 P01ind;	//indicies of (right, left) node in vls.C array
...
public:
	...
	Array1Dv2us P01ind;	//indicies of (right, left) node in vls.C array
