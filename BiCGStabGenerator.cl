...
///////////////////////////////////////////////////////////////////////////////////////
/////////////////                                                 /////////////////////
/////////////////    BiConjugate Gradient Stabilized Kernels      /////////////////////
/////////////////                                                 /////////////////////

...
// Values needing to be defined for the csrmv kernels
// ROWBITS
// WGBITS
// WG_SIZE
// BLOCKSIZE
...
///////////////////////////////////////////////////////////////////////////////////////
/////////////////    CSR Sparse Matrix, Dense Vector Multiply     /////////////////////
///////////////////////////////////////////////////////////////////////////////////////
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void axpby_PCalc (const SIZE_TYPE size,
          __global double *__restrict__ pY,
          __global const double *__restrict__ pAlpha,
          const SIZE_TYPE pAlphaOffset,
          const SIZE_TYPE pOmegaOffset,
	...
	__global const double *__restrict__ pV )
{
    ...
    if (index >= size) return;
    const VALUE_TYPE alpha = *(pAlpha + pAlphaOffset);
    const VALUE_TYPE omega = *(pAlpha + pOmegaOffset);
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void axpby_xrCalc (const SIZE_TYPE size,
          __global double *__restrict__ pY,
          __global const double *__restrict__ pAlpha,
          const SIZE_TYPE pAlphaOffset,
          __global double *__restrict__ pZ,
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ReduceKernelBase_Sqr ( 
__global double *__restrict__ input,
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ReduceKernelBase_Dot ( 
__global double *__restrict__ inputA,
...
__local double *sdata)
{
	...
	sdata[tid] = inputA[stride] * inputB[stride] + inputA[stride+1] * inputB[stride+1];
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ReduceKernel_Sum_Final (
__global const double *__restrict__ input,
__global double *__restrict__ output,
__local double *sdata,
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ReduceKernel_Sum_Rho_Calc_Beta (
__global const double *__restrict__ input,
__global double *__restrict__ output,
__local double *sdata,
const SIZE_TYPE offsetRho,
...
;
	...
	if (tid == 0)
	{
		double rhoOld = output[offsetRho];output[offsetRho] = sdata[0];output[offsetBeta] = (sdata[0] / rhoOld)*(output[offsetAlpha]/output[offsetOmega]);;
	}
}
#undef SIZE_TYPE
#undef WG_SIZE
...
;
	...
	if (tid == 0)
	{
		output[offsetAlpha] = output[offsetRho] / sdata[0];;
	}
}
#undef SIZE_TYPE
#undef WG_SIZE
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ElementWise_Add (const SIZE_TYPE size,
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ElementWise_Subtract (const SIZE_TYPE size,
                __global VALUE_TYPE* pR,
                __global const VALUE_TYPE* pX,
                __global const VALUE_TYPE* pY)
{
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ElementWise_Multiply (const SIZE_TYPE size,
                __global VALUE_TYPE* pR,
                __global const VALUE_TYPE* pX,
                __global const VALUE_TYPE* pY)
{
...
__kernel
__attribute__((reqd_work_group_size(WG_SIZE,1,1)))
void ElementWise_Divide (const SIZE_TYPE size,
                __global VALUE_TYPE* pR,
                __global const VALUE_TYPE* pX,
                __global const VALUE_TYPE* pY)
{
